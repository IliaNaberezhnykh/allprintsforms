...
Функция СведенияОВнешнейОбработке()Экспорт
	...
	ПараметрыРегистрации.Вставить("Наименование", "ОИР Счет-оферта"); 
	ПараметрыРегистрации.Вставить("БезопасныйРежим", ЛОЖЬ);
	ПараметрыРегистрации.Вставить("Версия", "1.0"); 
	ПараметрыРегистрации.Вставить("Информация", "ОИР Счет-оферта"); 
	ТаблицаКоманд = ПолучитьТаблицуКоманд();
	ДобавитьКоманду(ТаблицаКоманд, "Счет-оферта", "СчетНаОплату", "ВызовСерверногоМетода", Истина, "ПечатьMXL");
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Функция Печать(МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
		...
		Для Каждого СтруктураОбъектов Из МассивОбъектов Цикл 
			...
			Если ТипЗнч(СтруктураОбъектов.Ссылка) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
	...
	Процедура ЗаполнитьРеквизитыШапкиСчетаНаОплату(ДанныеПечати, Макет, ТабличныйДокумент, ТаблицаЭтапыОплаты, ТаблицаТовары)
		...
		Иначе
			...
			ИначеЕсли  ТипЗНЧ(ДанныеПечати.Ссылка) = Тип("ДокументСсылка.СчетНаОплатуКлиенту")  Тогда 
				...
				Иначе
					ДокЗаказ = ДанныеПечати.Ссылка.ДокументОснование;  // значит основание заказ клиента
				...
				Иначе 
					Если  ТипЗнч(ДокЗаказ) = Тип("ДокументСсылка.ЗаказКлиента") и ДанныеПечати.Ссылка.ЧастичнаяОплата Тогда
						...
						Если ПроцАванс <> Неопределено Тогда 
							...
							//ДанныеПечати.НазначениеПлатежа =  СтруктураДанныхШапки.НазначениеПлатежа;
						КонецЕсли;
					...
					ИначеЕсли   ТипЗнч(ДанныеПечати.Ссылка.ДокументОснование) =  Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		ТекстЗаголовка = ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(ДанныеПечати, НСтр("ru='Счет-оферта на оплату'"));
		СтруктураДанныхШапки = Новый Структура;
		...
		Если ТипЗнч(ДанныеПечати.Ссылка) = Тип("ДокументСсылка.СчетНаОплатуКлиенту") и ЗначениеЗаполнено(ДанныеПечати.Ссылка.ДокументОснование) Тогда 
			...
			ФормированиеПечатныхФорм.ОписаниеОрганизации(ФормированиеПечатныхФорм.СведенияОЮрФизЛице(Отправитель, ДанныеПечати.Дата),
			"ПолноеНаименование,ИНН,КПП,ЮридическийАдрес,Телефоны,"));
			ОбластьМакета.Параметры.Заполнить(СтруктураДанныхПоставщик);
			ТабличныйДокумент.Вывести(ОбластьМакета);
			...
			Если ЗначениеЗаполнено(ДокЗаказ.АдресГрузополучателя) Тогда
				...
				"ПолноеНаименование,ИНН,КПП,")+ ", " + ДокЗаказ.АдресГрузополучателя; 
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ДанныеПечати.Грузоотправитель) Тогда
				...
				СтруктураДанныхГрузоотправитель.Вставить("ПредставлениеГрузоотправителя", ОписаниеОрганизации(ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.ОрганизацияПоставщик, ДанныеПечати.Дата),
				"ПолноеНаименование,ИНН,КПП,ЮридическийАдрес,Телефоны,"));
				ТабличныйДокумент.Вывести(ОбластьМакета);
			иначе
				ОбластьМакета = Макет.ПолучитьОбласть("Грузоотправитель");
			...
			Иначе
				Если ЗначениеЗаполнено(ДокЗаказ.АдресГрузополучателя) Тогда
					...
					//// ДанныеПечати.Ссылка.АдресГрузополучателя)); //
					//ОбластьМакета.Параметры.Заполнить(СтруктураДанныхГрузополучатель);
					Если Не ЗначениеЗаполнено(ДокЗаказ.Грузополучатель) Тогда
					...
					"ПолноеНаименование,ИНН,КПП,")+ ", " + ДокЗаказ.АдресГрузополучателя + ", " +  ФормированиеПечатныхФорм.ОписаниеОрганизации(ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.Контрагент, ДанныеПечати.Дата),
					"Телефоны");
				Иначе
					ГрузСТрока = ФормированиеПечатныхФорм.ОписаниеОрганизации(ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.Грузополучатель, ДанныеПечати.Дата),
					"ПолноеНаименование,ИНН,КПП,")+ ", " + ДокЗаказ.АдресГрузополучателя + ", " +  ФормированиеПечатныхФорм.ОписаниеОрганизации(ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.Грузополучатель, ДанныеПечати.Дата),
					"Телефоны")
					КонецЕсли;
					ОбластьМакета.Параметры.ПредставлениеГрузополучателя = ГрузСТрока;
					//--БИТВРН Кравец Ю.А. 24.01.2022
	...
	Процедура ЗаполнитьРеквизитыПодвалаСчетаНаОплату(ДанныеПечати, Макет, ТабличныйДокумент, ТаблицаЭтапыОплаты, СоответствиеСтавокНДС, ПараметрыПечати)
		...
		//Оферта
		ОбластьОферта = Макет.ПолучитьОбласть("Оферта");
		Если ДокЗаказ <> Тип("ДокументСсылка.ЗаказКлиента") Тогда
		ОбластьОферта.Параметры.АдресГрузополучателя = ФормированиеПечатныхФорм.ОписаниеОрганизации(ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДанныеПечати.Контрагент, ДанныеПечати.Дата),
						"ЮридическийАдрес");
	Иначе
		ОбластьОферта.Параметры.АдресГрузополучателя = ДокЗаказ.АдресДоставки;
        КонецЕсли;
		
		ОбластьОферта.Параметры.Заполнить(ДанныеПечати);
		Если ТипЗНЧ(ДанныеПечати.Ссылка) = Тип("ДокументСсылка.СчетНаОплатуКлиенту") Тогда
		ОбластьОферта.Параметры.Грузоотправитель = ДанныеПечати.Ссылка.ДокументОснование.Грузоотправитель.НаименованиеПолное;
		ОбластьОферта.Параметры.Грузополучатель = ДанныеПечати.Ссылка.ДокументОснование.Грузополучатель.НаименованиеПолное;	
		Иначе
		ОбластьОферта.Параметры.Грузоотправитель = ДанныеПечати.Грузоотправитель.НаименованиеПолное;
		ОбластьОферта.Параметры.Грузополучатель = ДанныеПечати.Грузополучатель.НаименованиеПолное;
	КонецЕсли;
		Если ТипЗНЧ(ДанныеПечати.Ссылка) = Тип("ДокументСсылка.СчетНаОплатуКлиенту") Тогда
	    ОбластьОферта.Параметры.ИнформацияСчета = ДанныеПечати.ДополнительнаяИнформация;
		КонецЕсли;
	...
	Процедура ЗаполнитьТабличныйДокументСчетаНаОплату(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПараметрыПечати, ТоварыРеализации = Неопределено)
		...
		//ФормированиеПечатныхФорм.ИмяДополнительнойКолонки());	было в 2.4
	...
	Функция ОписаниеОрганизации(ДанныеПечати, ИмяОрганизации)
		...
		РеквизитыОрганизации = "ПолноеНаименование,КПП,ФактическийАдрес,ЮридическийАдрес,Телефоны,";
