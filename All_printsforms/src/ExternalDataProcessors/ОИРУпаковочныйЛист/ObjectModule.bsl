...
Функция ПолучитьДанныеДляПечати(Ссылка)
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		ЗапросЗК.УстановитьПараметр("Ссылка", Ссылка);
		Результат = ЗапросЗК.Выполнить(); 
		Выборка = Результат.Выбрать();
	КонецЕсли;	 
	...
	Если Выборка.Следующий() Тогда
		...
		Ссылка.Организация.ИНН,
		Ссылка.Организация.КПП));
		АдресПродавца = "";
		Для Каждого Стр Из Ссылка.Организация.КонтактнаяИнформация Цикл
			Если Стр.Вид.Наименование = "Международный адрес для платежей (англ.)" Тогда
				АдресПродавца = Стр.Представление;
			КонецЕсли;
		КонецЦикла;	
		СведенияОПродавце = ФормированиеПечатныхФорм.СведенияОЮрФизЛице(Выборка.Организация, Выборка.ДатаРТУ);
		ДанныеШапка.Вставить("ОрганизацияПродавецАнгл",Ссылка.Организация.НаименованиеМеждународное);
		ДанныеШапка.Вставить("АдресОрганизацииАнгл", АдресПродавца);
		ДанныеШапка.Вставить("ИННКПП", СтрШаблон("%1/%2",Ссылка.Организация.ИНН,Ссылка.Организация.КПП));
		...
		ДанныеШапка.Вставить("ИННКПППокупателя", СтрШаблон("%1/%2",Ссылка.Контрагент.ИНН,Ссылка.Контрагент.КПП));
		АдресПокупателя = "";
		Для Каждого Стр Из Ссылка.Контрагент.КонтактнаяИнформация Цикл
			Если Стр.Вид.Наименование = "Международный адрес" Тогда
				АдресПокупателя = Стр.Представление;
			КонецЕсли;
		КонецЦикла;										
		СведенияОПокупателе = ФормированиеПечатныхФорм.СведенияОЮрФизЛице(Выборка.Контрагент, Выборка.ДатаРТУ); 
		ДанныеШапка.Вставить("ПокупательАнгл", Ссылка.Контрагент.НаименованиеМеждународное);
		...
		ДанныеШапка.Вставить("ОснованиеАнгл", Ссылка.Договор.НаименованиеИностранное); 
		...
		Для Каждого Стр Из Ссылка.Грузополучатель.КонтактнаяИнформация Цикл
			Если Стр.Вид.Наименование = "Международный адрес для платежей (англ.)" Тогда
				ТвойАдресГрузополучателя = Стр.Представление;
			КонецЕсли;
		КонецЦикла;
		ГрузополучательМеждународный = Ссылка.Грузополучатель.НаименованиеМеждународное;
		...
		ДанныеШапка.Вставить("ГрузополучательАнгл", Ссылка.Грузополучатель.НаименованиеМеждународное); 
		...
		Если ЗначениеЗаполнено(Выборка.Грузоотправитель) Тогда
			...
			ПредставлениеГрузоотправителяАнгл = Ссылка.Организация.НаименованиеМеждународное + ", INN " + Ссылка.Организация.ИНН + ", " + АдресПродавца;
		...
		ДанныеШапка.Вставить("ОтправительСокрАнгл", Ссылка.Организация.НаименованиеМеждународное);
		ДанныеШапка.Вставить("ПолучательСокрАнгл", Ссылка.Контрагент.НаименованиеМеждународное); 
		ДанныеДляПечати.Вставить("ДанныеШапка", ДанныеШапка);
	КонецЕсли;
