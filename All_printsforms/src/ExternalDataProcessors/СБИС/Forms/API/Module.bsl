...
Функция сбисВыполнитьДействие(Кэш, СоставПакета, Этап, Действие, Комментарий, ПредставлениеПакета) Экспорт
	...
	Результат = Кэш.ОбщиеФункции.СбисВыполнитьДействие(Кэш, Новый Структура("СоставПакета, Этап, Действие, Комментарий, ПредставлениеПакета, Сертификат, СертификатДляПодписания, Алгоритм", СоставПакета, Этап, Действие, Комментарий, ПредставлениеПакета, Сертификат, СертификатДляПодписания, Алгоритм), Отказ);
	Если Отказ Тогда
...
Функция ОбработатьСлужебныеДокументыПоПакету(Кэш, СоставПакета) Экспорт
	...
	Для Каждого Док Из Результат.Документ Цикл
		...
		Если Действие.ТребуетПодписания="Да" Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Выполнено = Кэш.ОбщиеФункции.СбисВыполнитьДействие(Кэш, Новый Структура("СоставПакета, Этап, Действие, Комментарий, Сертификат, СертификатДляПодписания, Алгоритм", Док, Док.Этап[0], Действие, "", Сертификат, СертификатДляПодписания, Алгоритм), Отказ);
		Если Отказ Тогда
...
Функция Включить(Кэш, ДопПараметры=Неопределено, Отказ=Ложь) Экспорт
	...
	Кэш.СБИС.ПараметрыИнтеграции.ИнтеграцияИмя	= "API";
...
Функция Интеграция_ФЭДМультиСгенерировать(ПараметрыДокумента, НаборПодстановок, ДопПараметры) Экспорт
	...
	Попытка
		...
		Если Отказ Тогда
			Возврат МодульОбъектаКлиент().ВызватьСбисИсключение(Результат, Кэш.СБИС.ПараметрыИнтеграции.ИнтеграцияИмя + ".СбисОтправитьИОбработатьКомандуCallSabyApi2");
		КонецЕсли;
