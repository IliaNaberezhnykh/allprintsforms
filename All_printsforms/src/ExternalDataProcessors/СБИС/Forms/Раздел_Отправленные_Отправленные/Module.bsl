...
Функция ПодготовитьСтруктуруДокументаДляРасхождений(СтрокаСпискаДокументов, Кэш) Экспорт
	...
	МассивРазобранныхПакетов = Новый Массив(); Соответствие = Новый Соответствие();
	МассивПакетов = Кэш.СБИС.МодульОбъектаКлиент.ПодготовитьСтруктуруДокументаСбисДляРасхождений(СтрокаСпискаДокументов, Кэш);
	Для Каждого Пакет Из МассивПакетов Цикл 
		...
		МассивРазобранныхПакетов.Добавить(ПолныйСоставПакета);
		
		Для Каждого Вложение Из ПолныйСоставПакета.Вложение Цикл
			МассивДокументов1С = Новый Массив();
			ИниВложение = Кэш.ФормаНастроек.Ини(Кэш, Вложение.ИмяИни);
			Если	Не Вложение.Свойство("Направление")
				...
				Возврат "Проверка документа не поддерживается.";
			ИначеЕсли Не ЗначениеЗаполнено(СтруктураИниФайла) Тогда
				Возврат "В настройках не указан тип документа 1С, которому соответствует данный файл.";
			ИначеЕсли Не ИниВложение.Свойство("мДокумент", мДокументы) Тогда
				Возврат "Проверка документа не поддерживается.";
			КонецЕсли;
			СтруктураДокументаПоДаннымСбис = Вложение.СтруктураФайла.Файл.Документ;
			СтруктураДокументаПоДанным1С = Кэш.СБИС.МодульОбъектаКлиент.ПолучитьСтруктуруДокумента1СПоПакетуСбис(Новый Структура("Вложение, СоставПакета", Вложение, ПолныйСоставПакета), Новый Структура("Кэш", Кэш));
			Если Не СтруктураДокументаПоДанным1С = Неопределено Тогда
				Соответствие.Вставить(Вложение.Название, Новый Структура("СтруктураДокументаПоДаннымСбис, СтруктураДокументаПоДанным1С", СтруктураДокументаПоДаннымСбис, СтруктураДокументаПоДанным1С));
			КонецЕсли;	
		КонецЦикла;	
	КонецЦикла;
