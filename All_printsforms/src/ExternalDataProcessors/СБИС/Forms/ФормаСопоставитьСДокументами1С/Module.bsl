...
Процедура ТаблицаСопоставленияДокументовВыбор(Элемент, ВыбраннаяСтрока, Колонка, СтандартнаяОбработка)
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		ИмяКолонки = "ТаблицаСопоставленияДокументов" + Колонка.Имя;
	#Иначе
	...
	ИначеЕсли Лев(ИмяКолонки, 52) = "ТаблицаСопоставленияДокументовСостояниеСопоставления" Тогда
		...
		Если ВыбраннаяСтрока.СостояниеСопоставления = 1 Тогда
		ИначеЕсли ВыбраннаяСтрока.СостояниеСопоставления = 2 Тогда
			 фрм = МестныйКэш.ГлавноеОкно.сбисПолучитьФорму("ФормаПроблемСопоставления",,,ЭтаФорма);			 
			 фрм.Показать(МестныйКэш, ВыбраннаяСтрока);
		КонецЕсли;
...
Процедура СохранитьНажатие(Элемент)
	...
	Для Каждого Строка из ТаблицаСопоставленияДокументов Цикл
		Если Строка.Документ1С<>Неопределено и Не Строка.Документ1С.Пустая() Тогда
			...
			фрм.ЗаписатьПараметрыДокументаСБИС(СтруктураСвойств, Строка.Документ1С, МестныйКэш.Ини.Конфигурация, ГлавноеОкно.КаталогНастроек, Новый Структура("ИдАккаунта", СбисИдАккаунта));
