...
Функция УстановитьПараметрыМодуля(ПараметрыУстановки=Неопределено, Кэш)				Экспорт
	ЛокальныйКэш = Кэш;
	УстановитьПараметрыРегистры(Кэш);	
    Попытка
...
Функция СбисПрочитатьЗначенияСвойствДокументов(ПараметрыЗапросаВходящие, Кэш)		Экспорт
	...
	ЭтоСписокЗначений		= ТипЗнч(СписокСсылок) = Тип("СписокЗначений");
	...
	ВыборкаСвойств = ПрочитатьСвойстваДокументовПоСсылкеНаСервере(СписокСсылок, СписокСвойствДляЗапроса);
	Результат = Новый Соответствие;
	Для Каждого СсылкаЗапрос Из СписокСсылок Цикл
		ЗначениеСвойств = ВыборкаСвойств.Получить(?(ЭтоСписокЗначений, СсылкаЗапрос.Значение, СсылкаЗапрос));
...
//		ЗаполнитьСумму - булево, признак необходимости извлечения реквизита "СуммаДокумента".
&НаКлиенте
Процедура ЗаполнитьСписокДокументовСБИССвязьюСсылок1С(ПараметрыСпискаДокументов, Кэш)
...
&НаСервере
Функция ПрочитатьСвойстваДокументовПоСсылкеНаСервере(СписокДокументов, СписокСвойств)
	Запрос = Новый Запрос;
