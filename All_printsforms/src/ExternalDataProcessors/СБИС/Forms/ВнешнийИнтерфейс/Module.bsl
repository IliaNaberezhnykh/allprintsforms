...
Функция сбисОбновлениеСтатусов(Кэш, ДопПараметры = Неопределено) Экспорт	
	Перем Метки, ЗначМетки;
	МеткиПараметры = "ДатаПоследнегоЗапросаСтатусов
						|ИдентификаторПоследнегоСобытия
						|ДатНачЧтенияСтатусов
						|ДатКнцЧтенияСтатусов";
	Если ТипЗнч(ДопПараметры) = Тип("Структура") 
			И ДопПараметры.Свойство("Метки", Метки) Тогда
		Если Не ТипЗнч(Метки) = Тип("Структура")  Тогда
			Возврат ("Ошибка во входящем параметре ""Метки"". Тип должен быть структура.");
		КонецЕсли;
	Иначе
		Попытка
			Метки = Кэш.ФормаНастроек.СбисПрочитатьМеткиСтатусов(Кэш);
		Исключение 
			Возврат (Метки.Message + " (" + Метки.Details + ")");
		КонецПопытки;
	КонецЕсли;
	Для НомерПараметра = 1 По СтрЧислоСтрок(МеткиПараметры) Цикл
		ИмяПараметра = СтрПолучитьСтроку(МеткиПараметры, НомерПараметра);
		Если Метки.Свойство(ИмяПараметра, ЗначМетки) Тогда
			Кэш.ГлавноеОкно[ИмяПараметра] = ЗначМетки;
		КонецЕсли;
	КонецЦикла;
	Попытка
		Кэш.Интеграция.ПолучитьСписокИзменений(Кэш, ДопПараметры);
	Исключение
		ИнфОбОшибке = ИнформацияОбОшибке();
		СбисИсключение = Кэш.СБИС.МодульОбъектаКлиент.НовыйСбисИсключение(ИнфОбОшибке, "ВнешнийИнтерфейс.сбисОбновлениеСтатусов");
		Возврат СбисИсключение.Message + " (" + СбисИсключение.Details + ")";
	КонецПопытки;
    Кэш.СБИС.МодульОбъектаКлиент.СохранитьМеткиСтатусов(Кэш);
	Если ТипЗнч(ДопПараметры) = Тип("Структура") Тогда
		Для НомерПараметра = 1 По СтрЧислоСтрок(МеткиПараметры) Цикл
			ИмяПараметра = СтрПолучитьСтроку(МеткиПараметры, НомерПараметра);
			Если Метки.Свойство(ИмяПараметра) Тогда
				Метки[ИмяПараметра] = Кэш.ГлавноеОкно[ИмяПараметра];
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
КонецФункции
