...
Функция ЗагрузитьАПИ3Объект(Кэш, СтруктураОбъекта, ИдентификаторПосылки, Отказ)
	...
	Для Каждого ОбъектНаЗапись Из ОбъектыНаЗапись Цикл
		...
		Если ИмяОбъекта = ИмяСБИС Тогда
	        ...
	        фрм.ЗаписатьПараметрыДокументаСБИС(СтруктураСвойств, Результат.Ссылка, МестныйКэш.Ини.Конфигурация, МестныйКэш.ГлавноеОкно.КаталогНастроек, Новый Структура("ИдАккаунта", СбисИдАккаунта));
		КонецЕсли;
	КонецЦикла;
