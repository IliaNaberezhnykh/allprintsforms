...
Процедура СбисИнициироватьФорму(Кэш) Экспорт
	...
	МестныйКэш				= Кэш;
	ПараметрыРаботыФормы	= Новый Структура;
...
Процедура КонфигурацияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ПараметрыРаботыФормы.Вставить("КонфигурацияБыла", Конфигурация);
...
Процедура КонфигурацияПриИзменении_Завершение(АргументЗавершения, ДопПараметры) Экспорт
	...
	Если лОткатитьСистему Тогда
		Конфигурация = ПараметрыРаботыФормы.КонфигурацияБыла;
	КонецЕсли;
...
Функция	СформироватьСтруктуруРазделов(Кэш)
	СтруктураРазделовxml = ПолучитьСтруктуруАккордеона();
	СтруктураРазделов = Кэш.ГлавноеОкно.DecodeXmlXdto(СтруктураРазделовxml);
	Возврат СтруктураРазделов;	
КонецФункции

&НаСервере
Функция ПолучитьСтруктуруАккордеона()
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		ОбработкаОбъект = ЭтотОбъект;
	#Иначе
		ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	#КонецЕсли
    Возврат ОбработкаОбъект.ПолучитьМакет("СтруктураАккордеона").ПолучитьТекст();
КонецФункции
...
Функция		УстановитьИниКонфигурации(Кэш, ИниКонфигурации, ДополнительныеПараметры=Неопределено, Отказ) Экспорт
	...
	Попытка
		//UAA НоваяЛента
		ФормаРаботыСоСтатусамиЗначение.УстановитьПараметрыМодуля(,Кэш);
	Исключение
...
Процедура СбисПолучитьИУстановитьНастройкиВКэш(ПараметрыУстановкиВходящие, ЛокальныйКэш) Экспорт
	...
	Попытка
		...
		КонецЕсли;	
		СтруктураМетокСтатусов = ЛокальныйКэш.ФормаНастроек.СбисПрочитатьМеткиСтатусов(ЛокальныйКэш);
		ЗаполнитьЗначенияСвойств(ЛокальныйКэш.ГлавноеОкно, СтруктураМетокСтатусов);
	Исключение
		ИнфоОбОшибке = ИнформацияОбОшибке();
		МодульОбъектаКлиент().ВызватьСбисИсключение(ИнфоОбОшибке, "ФайлНастроекОбщее.СбисПолучитьИУстановитьНастройкиВКэш");
	КонецПопытки;
КонецПроцедуры
...
Процедура СбисУстановитьНастройки(ПараметрыУстановкиВходящие, ЛокальныйКэш)  Экспорт
	...
	Исключение
		ЛокальныйКэш.ГлавноеОкно.СбисСпрятатьСостояние();
		ИнфоОбОшибке = ИнформацияОбОшибке();
		МодульОбъектаКлиент().ВызватьСбисИсключение(ИнфоОбОшибке, "ФайлыНастроекОбщее.СбисУстановитьНастройки");		
	КонецПопытки;
КонецПроцедуры
