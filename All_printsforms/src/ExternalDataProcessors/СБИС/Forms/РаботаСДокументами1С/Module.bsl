	...
	Если ФормаПросмотра.СоставПакета.Свойство("Вложение") Тогда
		...
		Для Каждого Документ Из Документы1С Цикл
			...
			Иначе
				...
				ОткрытьФорму(МестныйКэш.ГлавноеОкно.СбисПутьКФормамОбработки() + "Форма_ВыборИзСписка",Новый Структура("СписокДляВыбора, Заголовок",СписокИни, "Выберите настройку для формирования документов для "+Документ.Значение),,,,,Оповещение);
			КонецЕсли;
		КонецЦикла;
...
Функция ПолучитьСторону(Кэш, знач Контекст) Экспорт
	...
	//Параметры, которые не должны попасть в сторону, но необходимы для рассчёта.
	...
	Если	Контекст.Свойство("Роль")
		И	Контекст.Роль = "Получатель" Тогда
		 ДопПараметрыДляСторны.Вставить("ИспользоватьКакПолучателя", РассчитатьЗначение("ИспользоватьКакПолучателя",Контекст) = Истина);
	КонецЕсли;
	...
	Если СтрДлина(СокрЛП(Инн))<>12 И НЕ ИностраннаяОрганизация Тогда
		...
		Если Контекст.Свойство("КПППодразделения") и ЗначениеЗаполнено(Контекст.КПППодразделения) // в счет-фактуре КПП контрагента можно указать прямо на документе (если у контрагента есть филиалы)
				И Не ДопПараметрыДляСторны.Получить("ИспользоватьКакПолучателя") = Истина Тогда  
	...
	КонецЕсли;
	Если ДопПараметрыДляСторны.Количество() Тогда
		КлючДляПараметров = КлючСтороныДляДопПараметров(Участник, Новый Структура);
		Кэш.ТекущийСеанс.РасчитанныеЗначения.мСторона.Вставить(КлючДляПараметров, ДопПараметрыДляСторны);
	КонецЕсли;
...
Функция ИтогСуммыПоТаблДок(ДокументСсылка, КурсВзаиморасчетов, КратностьВзаиморасчетов, РаспределяемаяСумма)
	...
	Пока Выборка.Следующий() Цикл  
		...
		СуммаБазы = СуммаБазы + Выборка.Сумма; 
	КонецЦикла;
...
Функция СгенерироватьИсходящийПакет(ПакетВходящий, ПараметрыГенерацииПакета)
	...
	ПараметрыГенерацииВложений	= Новый Структура("ОснованиеПакета, СоставПакета, Результат", ОснованиеПакета, ПакетВходящий, Результат);
...
Функция СгенерироватьНаборВложенийВПакет(ПараметрыСгенерироватьВходящие, Кэш)
	...
	Если Не Вложение.Свойство("Документы1С", Документы1СВложения) Тогда
		...
		Документы1СВложения.Добавить(?(Вложение.Свойство("Документ1С"),Вложение.Документ1С, ПараметрыСгенерироватьВходящие.ОснованиеПакета));
	КонецЕсли;
	...
	ИначеЕсли	Вложение.Свойство("СтруктураДокумента", СтруктураФайла) Тогда 
		...
		Если ГенераторВключенДляДокумента(Кэш, Вложение) Тогда
			ШаблонXML = "";
			фрм = Кэш.ГлавноеОкно.сбисНайтиФормуФункции("ПрименитьПодстановку", "Файл_Шаблон", "Файл_Шаблон_" + СтрЗаменить(Вложение.ВерсияФормата, ".", "_"), Кэш);
			Если фрм = Ложь Тогда
				МодульОбъектаКлиент().ВызватьСбисИсключение(, "РаботаСДкоументами1С.СгенерироватьДанныеВложенийВПакет", 500, , "Не определен модуль генерации вложения.");
			Иначе
				...
				Если ОшибкаФормирования Тогда
					МодульОбъектаКлиент().ВызватьСбисИсключение(РезультатПодставновки, "РаботаСДкоументами1С.СгенерироватьДанныеВложенийВПакет");
				ИначеЕсли ТипЗнч(РезультатПодставновки) = Тип("Массив") Тогда
					Для Каждого ВложениеПодстановки Из РезультатПодставновки Цикл
						...
						Иначе
							СтруктураВложенияДобавить = СкопироватьОбъектСПараметрамиКлиент(СтруктураВложения,, Ложь);
							...
							Если	СтруктураВложения.Свойство("СтруктураФайла")
								...
								СтруктураВложенияДобавить.СтруктураФайла			= СкопироватьОбъектСПараметрамиКлиент(СтруктураВложения.СтруктураФайла,, Ложь);
								СтруктураВложенияДобавить.СтруктураФайла.Файл		= СкопироватьОбъектСПараметрамиКлиент(СтруктураВложения.СтруктураФайла.Файл,, Ложь);
		...
		Иначе
			...
			СтруктураВложения.Вставить("XMLДокумента", сбисПолучитьXMLФайлаИзСтруктуры(Кэш, СтруктураВложения));
...
Функция ГенераторВключенДляДокумента(Кэш, ФайлДанные) Экспорт
	Возврат		Кэш.Парам.ИспользоватьГенератор = Истина
			И	Кэш.СБИС.ПараметрыИнтеграции.ГенераторФЭД
	...
	Если СоставПакета.Свойство("Вложение", ПакетВложения) Тогда
		Для Каждого Вложение Из ПакетВложения Цикл
			Если Не (Вложение.Свойство("ТребуетПодписания") И Вложение.ТребуетПодписания) Тогда
				Продолжить;
			КонецЕсли;
			ВложениеЗаписано = Кэш.Интеграция.СБИСЗаписатьВложения(Кэш,СоставПакета, Вложение);
			Если Не ВложениеЗаписано Тогда
				Возврат ВложениеЗаписано;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
Функция ИдентификаторСБИСПоДокументу(Кэш, ДокументСсылка) Экспорт
	...
	фрм = Кэш.ГлавноеОкно.СбисНайтиФормуФункции("СбисПрочитатьСведенияОПакете", Кэш.ФормаРаботыСоСтатусами, "", Кэш);
	Возврат фрм.СбисПрочитатьСведенияОПакете(ПараметрыПрочитатьСвойство, Кэш);
	
	//фрм = Кэш.ГлавноеОкно.СбисНайтиФормуФункции("СбисПрочитатьЗначениеСвойстваДокумента", Кэш.ФормаРаботыСоСтатусами, "", Кэш);
	//Возврат фрм.СбисПрочитатьЗначениеСвойстваДокумента(ПараметрыПрочитатьСвойство, Кэш);
