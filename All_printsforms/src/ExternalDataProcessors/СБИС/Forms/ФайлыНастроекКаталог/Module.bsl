...
&НаКлиенте
Функция		ПрочитатьОбщиеНастройкиПодключения(Кэш, КаталогНастроек, Отказ=Ложь) Экспорт
	Возврат  ПолучитьПараметрыСБИС(Кэш, КаталогНастроек, Отказ);
КонецФункции 
...
Функция СбисСохранитьМеткиСтатусов(Кэш, МеткиСтатусов, ДопПараметры = Неопределено, Отказ = Ложь) Экспорт
	// Как-то получаем параметр status_marks
