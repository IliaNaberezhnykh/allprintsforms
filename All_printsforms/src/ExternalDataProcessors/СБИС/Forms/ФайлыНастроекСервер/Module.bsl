...
Функция		ПолучитьСБИСПараметрыСоединения(Кэш, Идентификатор, Отказ)
	...
	//Читаем с шаблона параметры, хранящиеся на шаблоне
	Если  Не Результат.ПараметрыРаботы.ТипНастроек = "Шаблон" И ЗначениеЗаполнено(Результат.ПараметрыРаботы.Шаблон) Тогда	
		ИдШаблона = Результат.ПараметрыРаботы.Шаблон;
		ШаблонПодключения = НовыйIntegrationConnection(Кэш, Новый Структура("Идентификатор, ИнициироватьДанные", ИдШаблона, Ложь), Ложь);
	КонецЕсли;	
	Если Не ШаблонПодключения = Неопределено Тогда
		Для Каждого Элемент Из ШаблонПодключения.Данные.Параметры Цикл
			Если Не Кэш.КэшНастроек.СписокПараметровХранящихсяНаШаблоне.Найти(нрег(Элемент.Ключ)) = Неопределено Тогда
...
Функция		_КлассIntegrationConnection_ОсновныеПоля(Кэш)
	...
	,ПараметрыРаботыIntegrationConnection,	Новый Структура("Ини, Меню, Параметры"),"");	
	Возврат Результат;
КонецФункции
...
Функция		_КлассIntegrationConnection_ЗаполнитьПоДанным(Кэш, КлассIntegrationConnection, ДопПараметры, Отказ)
			...
			Попытка
				...
				Для	Каждого КлючИЗначение Из Соединениеdata Цикл
					...
					Если	КонтролироватьЧтениеИни Тогда
						ИндексВКонтроле = СписокНаКонтроль.НайтиПоЗначению(НРег(ИмяИни));
	...
	Если	КонтролироватьЧтениеИни
		...
		Для Каждого  ЭлементНаКонтроль Из СписокНаКонтроль Цикл
			СтрокаОшибки = СтрокаОшибки + ЭлементНаКонтроль.Представление + ", ";
		КонецЦикла;
...
Функция		КлассIntegrationConnection_Инициализировать(Кэш, ДопПараметры, Отказ)
	...
	Если ЗначениеЗаполнено(СписокИниПолучить) Тогда
		СписокНаКонтроль = Новый СписокЗначений;
		Для Каждого ЭлементСпискаПолучить Из СписокИниПолучить Цикл
			СписокНаКонтроль.Добавить(НРег(ЭлементСпискаПолучить.name), ЭлементСпискаПолучить.name);
		КонецЦикла;
	...
	ИначеЕсли МассивПолучитьИтого.Количество() Тогда
		...
		Если Не ПолучитьТолькоПользовательскиеИзменения Тогда			
			...
			Для Каждого ЭлементСпискаПолучить Из МассивПолучитьИтого Цикл
				СписокНаКонтроль.Добавить(НРег(ЭлементСпискаПолучить.name), ЭлементСпискаПолучить.name);
			КонецЦикла;
			ПараметрыЗаполнения.Вставить("СписокКонтроль", СписокНаКонтроль);
		КонецЕсли;
...
Функция		КлассIntegrationConnection_СохранитьСбисПараметры(Кэш, КлассIntegrationConnection, ДопПараметры, Отказ)
	...
	СбисПараметры = Новый Структура();
	
	ЗаписатьШаблон = Ложь; ПараметрыКЗаписиНаШаблон = Новый Структура();
	Если  Не КлассIntegrationConnection.ПараметрыРаботы.ТипНастроек = "Шаблон" И ЗначениеЗаполнено(КлассIntegrationConnection.ПараметрыРаботы.Шаблон) Тогда	
		ИдШаблона = КлассIntegrationConnection.ПараметрыРаботы.Шаблон;
		ШаблонПодключения = НовыйIntegrationConnection(Кэш, Новый Структура("Идентификатор, ИнициироватьДанные", ИдШаблона, Ложь), Ложь);
	КонецЕсли;
		
	Для Каждого Элемент Из КлассIntegrationConnection.Данные.Параметры Цикл
		Если Не ШаблонПодключения = Неопределено 
			И Не Кэш.КэшНастроек.СписокПараметровХранящихсяНаШаблоне.Найти(нрег(Элемент.Ключ)) = Неопределено 
			И Не Элемент.Значение = Неопределено Тогда
			//Подготовка для записи на шаблон
			ПараметрыКЗаписиНаШаблон.Вставить(Элемент.Ключ, Элемент.Значение);
			ЗаписатьШаблон = Истина;
		Иначе
			//Преобразование даты к строке в корректном форматеи
			...
			Иначе
			//Или просто копируем в СбисПараметры	
				СбисПараметры.Вставить(Элемент.Ключ, Кэш.ОбщиеФункции.СбисСкопироватьОбъектНаКлиенте(Элемент.Значение));
			КонецЕсли;
		КонецЕсли;	
	КонецЦикла;
	
	Если ЗаписатьШаблон Тогда
		КлассIntegrationConnection_СохранитьСбисПараметры(Кэш, ШаблонПодключения, Новый Структура("ПараметрыДописать", ПараметрыКЗаписиНаШаблон), Ложь);
	КонецЕсли;	
...
&НаКлиенте
Функция СбисСохранитьМеткиСтатусов(Кэш, МеткиСтатусов, ДопПараметры = Неопределено, Отказ = Ложь) Экспорт
	...
	ИначеЕсли ДопПараметры.СпособХраненияМетокСтатусов = 1 Тогда
		Если Кэш.КэшНастроек.Свойство("ВыбранныеНастройки") И ЗначениеЗаполнено(Кэш.КэшНастроек.ВыбранныеНастройки) И ЗначениеЗаполнено(Кэш.КэшНастроек.ВыбранныеНастройки.ПараметрыРаботы.Шаблон) Тогда
			Попытка
	...
	Иначе
		...
		Кэш.ГлавноеОкно.МодульОбъектаКлиент().ОбщиеНастройкиСохранить(Результат, Новый Структура());
	КонецЕсли;
КонецФункции
...
#Область include_core_vo2_Настройки_СБИС_InitConnection
