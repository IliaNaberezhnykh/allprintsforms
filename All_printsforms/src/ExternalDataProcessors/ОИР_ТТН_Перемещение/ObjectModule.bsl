...
Функция СведенияОВнешнейОбработке()Экспорт
	...
	ПараметрыРегистрации.Вставить("Наименование", "ОИР ТТН (Перемещение)"); 
	ПараметрыРегистрации.Вставить("БезопасныйРежим", ЛОЖЬ);
	ПараметрыРегистрации.Вставить("Версия", "17.0"); 
	ПараметрыРегистрации.Вставить("Информация", "ОИР ТТН (Перемещение)"); 
	ТаблицаКоманд = ПолучитьТаблицуКоманд();
	ДобавитьКоманду(ТаблицаКоманд, "ОИР ТТН (Перемещение)", "ОИР ТТН (Перемещение)", "ВызовСерверногоМетода", Истина, "ПечатьMXL");
...
Процедура ЗаполнитьТабличныйДокументТН(ТабличныйДокумент, СтруктураДанных, ОбъектыПечати, КомплектыПечати)
	...
	Для Каждого ДанныеПечати Из ТаблицаДанныхДляПечати Цикл
		...
		СведенияОГрузополучателе  = ФормированиеПечатныхФорм.СведенияОЮрФизЛице(ДокОсн.Грузополучатель,  ДанныеПечати.Дата);
...
Процедура РассчитатьИтоговыеСуммыТТН(ИтоговыеСуммы, ДанныеСтроки, ТЗТТН = Неопределено, Номенклатура = Неопределено, СтрокаТовары = Неопределено, ДанныеПечати)
	ТекСтрокаРеал = ДанныеПечати.Ссылка.ДокументыОснования[0].ДокументОснование.ДокументОснование.ЗаказКлиента.Товары.НайтиСтроки(Новый Структура("Номенклатура", СтрокаТовары.Номенклатура));
	...
	ТекСтрока1 = ДанныеПечати.Ссылка.ДокументыОснования[0].ДокументОснование.ДокументОснование.ЗаказКлиента.Товары.НайтиСтроки(Новый Структура("Номенклатура", СтрокаТовары.Номенклатура)); 
	...
	Если  ДанныеПечати.Ссылка.ДокументыОснования[0].ДокументОснование.ДокументОснование.ЗаказКлиента.ЦенаВключаетНДС Тогда
...
Процедура ЗаполнитьРеквизитыСтрокиТовараТТН(ДанныеПечати, СтрокаТовары, ДанныеСтроки, ОбластьМакета, ЕдиницаИзмеренияВеса = Неопределено, КоэффициентПересчетаВТонны = 0, ТЗТТН = Неопределено,
	...
	ТекСтрока1 = ДанныеПечати.Ссылка.ДокументыОснования[0].ДокументОснование.Товары.НайтиСтроки(Новый Структура("Номенклатура", СтрокаТовары.Номенклатура));
	...
	Иначе
		НДСка = (1+(СтрЗаменить(Тееек.СтавкаНДС,"%","")/100));
	КонецЕсли;
	ТекСтрока1 = ДанныеПечати.Ссылка.ДокументыОснования[0].ДокументОснование.ДокументОснование.ЗаказКлиента.Товары.НайтиСтроки(Новый Структура("Номенклатура", ТекСтрока.Номенклатура));
	Если ТекСтрока1.Количество() Тогда
		Если ДанныеПечати.Ссылка.ДокументыОснования[0].ДокументОснование.ДокументОснование.ЗаказКлиента.ЦенаВключаетНДС Тогда                                   
...
Процедура ЗаполнитьРеквизитыШапкиТТН(ДанныеПечати, Макет, ТабличныйДокумент)
	...
	Если ТипЗнч(СсылкаНаРТУ) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда
	АдресГруз = ССылкаНаРТУ.ДокументОснование.ЗаказКлиента.АдресГрузополучателя;	
	Иначе
	...
	ГрузополучательМеждународный = ДокОсн.Грузополучатель.НаименованиеМеждународное;
...
Процедура ЗаполнитьТабличныйДокументТТН(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл   
		...
		ДокОсн = ДанныеПечати.Ссылка.ДокументыОснования[0].ДокументОснование.ДокументОснование; 
