...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	НоваяКоманда.Представление = НСтр("ru = 'ОИР Справка-расчет рублевых сумм документа в валюте'");
	НоваяКоманда.Идентификатор = "РублевыеСуммыДокументаВВалюте";
...
Функция СформироватьПечатнуюФормуСправкиРублевыеСуммыДокументаВВалюте(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	РеглУчетПроведениеСервер.ОтразитьДокументыВРеглУчетеПриПечати(МассивОбъектов);
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда	
		...
		|ГДЕ
		|	РасчетыСКлиентами.Регистратор В(&МассивСсылок)";
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			ЗаполнитьЗначенияСвойств(ОсновныеПараметры, ВыборкаДетальныеЗаписи);
			ОсновныеПараметры.ЭтоРасчетыСКлиентами = Истина;
			//ОсновныеПараметры.Регистратор = ВыборкаДетальныеЗаписи.Регистратор;		
	...
	Макет = ПолучитьМакет("ПФ_MXL_РублевыеСуммыДокументаВВалюте");
	...
	Для Каждого СтруктураОбъектов Из СтруктураТипов Цикл
		...
		Запрос.Текст = ПодготовитьТекстЗапросаДляПечатиСправкиРасчетаРублевыеСуммыДокументовВВалюте(НомераТаблиц);
...
Процедура ЗаполнитьТабличныйДокументСправки(ТабличныйДокумент, ВыборкаДокументов, ВыборкаСуммПоДокументам, ЕстьПредоплаты, ВыборкаПредоплат, ВалютаРегламентированногоУчета, Макет)
	...
	Если СуммаНеоплаченоВал <> 0 Тогда
		...
		ПараметрыТекущегоКурса.Вставить("Валюта",ВыборкаДокументов.ВалютаДокумента);
		...
		ПараметрыНовогоКурса.Вставить("Валюта",ВалютаРегламентированногоУчета);
		ПараметрыНовогоКурса.Вставить("Курс",1);
		ПараметрыНовогоКурса.Вставить("Кратность",1);
		
		СуммаНеоплаченоРуб = РаботаСКурсамиВалютКлиентСервер.ПересчитатьПоКурсу(СуммаНеоплаченоВал,ПараметрыТекущегоКурса,ПараметрыНовогоКурса);
	Иначе
	...
	//НачислятьНДСПоОтгрузке = УчетнаяПолитика.НачислятьНДСПоОтгрузке(ВыборкаДокументов.Организация, ВыборкаДокументов.Дата); = о
...
Функция ПодготовитьТекстЗапросаДляПечатиСправкиРасчетаРублевыеСуммыДокументовВВалюте(НомераТаблиц) Экспорт
	...
	|	ДанныеОбрабатываемогоДокумента.Валюта КАК ВалютаДокумента,
	|	ДанныеОбрабатываемогоДокумента.Организация КАК Организация,
	|	ДанныеОбрабатываемогоДокумента.Контрагент КАК Контрагент,
	|	ДанныеОбрабатываемогоДокумента.Договор КАК ДоговорКонтрагента,
	...
	|	ЕСТЬNULL(ДанныеОбрабатываемогоДокумента.Договор.УчетАгентскогоНДС, ЛОЖЬ) КАК УчетАгентскогоНДС,
	|	ДанныеОбрабатываемогоДокумента.ЦенаВключаетНДС КАК СуммаВключаетНДС
	|ПОМЕСТИТЬ ВТ_ТаблицаПоШапкеДокумента
	|ИЗ
	|	Документ.РеализацияТоваровУслуг КАК ДанныеОбрабатываемогоДокумента
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют КАК КурсыВалют
	|		ПО (НАЧАЛОПЕРИОДА(ДанныеОбрабатываемогоДокумента.Дата, ДЕНЬ) = КурсыВалют.Период)
	|			И ДанныеОбрабатываемогоДокумента.ВалютаВзаиморасчетов = КурсыВалют.Валюта
	|ГДЕ
	...
	|	РублевыеСуммыДокументовВВалюте.СуммаБезНДСРегл + РублевыеСуммыДокументовВВалюте.СуммаНДСРегл КАК Всего,
	|	РублевыеСуммыДокументовВВалюте.СуммаНДСРегл КАК НДС,
	|	РублевыеСуммыДокументовВВалюте.БазаНДСРегл КАК НалоговаяБазаНДС
	|ПОМЕСТИТЬ РегистрСведенийРублевыеСуммыДокументовВВалюте
	|ИЗ
	|	РегистрСведений.СуммыДокументовВВалютахУчета КАК РублевыеСуммыДокументовВВалюте
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	РасчетыСКлиентамиПоДокументам.Регистратор КАК Ссылка,
	|	СУММА(РасчетыСКлиентамиПоДокументам.ПредоплатаРегл) КАК СуммаПредоплатыРуб,
	|	СУММА(РасчетыСКлиентамиПоДокументам.Предоплата) КАК СуммаПредоплатыВал,
	|	РасчетыСКлиентамиПоДокументам.РасчетныйДокумент КАК Документ,
	|	ПРЕДСТАВЛЕНИЕ(РасчетыСКлиентамиПоДокументам.РасчетныйДокумент) КАК ДокументПредоплатыПредставление,
	|	ВТ_ТаблицаПоШапкеДокумента.Организация КАК Организация
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ТаблицаПоШапкеДокумента КАК ВТ_ТаблицаПоШапкеДокумента
	|		ПО РасчетыСКлиентамиПоДокументам.Регистратор = ВТ_ТаблицаПоШапкеДокумента.Ссылка
	...
	|	ОбрабатываемаяТаблица.АналитикаУчетаНоменклатуры.Номенклатура КАК Товар,
	|	ОбрабатываемаяТаблица.АналитикаУчетаНоменклатуры.Номенклатура.Наименование КАК ТоварНаименование,
	|	ОбрабатываемаяТаблица.СуммаСНДС КАК ВсегоВал,
	...
	|ИЗ
	|	Документ.РеализацияТоваровУслуг.ВидыЗапасов КАК ОбрабатываемаяТаблица
	...
	|			И ОбрабатываемаяТаблица.ИдентификаторСтроки = РегистрСведенийРублевыеСуммыДокументовВВалюте.ИдентификаторСтроки
	|ГДЕ
	|	ОбрабатываемаяТаблица.СуммаСНДС <> 0
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка,
	...
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РегистрНакопления.РасчетыСКлиентамиПоДокументам", "РегистрНакопления.РасчетыСКлиентамиПоСрокам");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РасчетыСКлиентамиПоДокументам.Регистратор", "РасчетыСКлиентамиПоДокументам.ДокументРегистратор");
	КонецЕсли;
