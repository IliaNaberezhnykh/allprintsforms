...
Процедура ВыполнитьКоманду(ИдентификаторКоманды, ОбъектыНазначенияМассив) Экспорт
	Для каждого СтрокаТЧ из ОбъектыНазначенияМассив цикл
	ОбъектЗаполнения = СтрокаТЧ;
	Объект.СсылкаИсточника = СтрокаТЧ;
	КонецЦикла;                 
	ЗакрытиеФормы = Новый ОписаниеОповещения("ЗаполнениеПоВыбраннымПараметрам", ЭтаФорма);
	ЗапросПараметров = ОткрытьФорму("ВнешняяОбработка.Excel.Форма",Новый Структура("Док", СтрокаТЧ) ,ОбъектЗаполнения,,,,ЗакрытиеФормы);
КонецПроцедуры
...
Процедура ЗаполнениеПоВыбраннымПараметрам(Результат, Параметры) Экспорт
	...
	ОбъектЗаполнения = ЗаполнениеНаСервере(ОбъектЗаполнения, Результат);
...
функция ЗаполнениеНаСервере(ОбъектЗаполнения, Параметры = Неопределено)
	...
	Попытка
		Док.ЭтотОбъект.Прочитать();//работает
	Исключение
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Док") Тогда
		Объект.СсылкаИсточника = Параметры.Док;
	КонецЕсли;
